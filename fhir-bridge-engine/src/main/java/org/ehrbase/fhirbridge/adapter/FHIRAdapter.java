...
public class FHIRAdapter {
    ...
    public String processFHIRRequest(String fhirRequestJson) {
        try {
            ...
            logger.info("Forwarding FHIR request to HAPI FHIR server...");
            HttpHeaders headers = new HttpHeaders();
            headers.setAccept(Arrays.asList(MediaType.APPLICATION_JSON));
            headers.setContentType(MediaType.APPLICATION_JSON);
            HttpEntity <String> entity = new HttpEntity<String>(fhirRequestJson, headers);
            
            String response = restTemplate.postForObject(hapiFhirUrl + "/fhir", entity, String.class);
            logger.info("Received response from HAPI FHIR server." + response);
            ...
            logger.error("Error while processing FHIR request with HAPI FHIR server.");
