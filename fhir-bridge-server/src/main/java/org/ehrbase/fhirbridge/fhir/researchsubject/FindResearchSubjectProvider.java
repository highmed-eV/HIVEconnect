 ...
 * for 'Find Research Subject' transaction.
 *
 * @since 1.0.0
 */
@SuppressWarnings({"unused", "java:S107", "DuplicatedCode"})
public class FindResearchSubjectProvider extends AbstractPlainProvider {

    @Search(type = ResearchSubject.class)
    public IBundleProvider searchResearchSubject(@OptionalParam(name = IAnyResource.SP_RES_ID) TokenAndListParam id,
                                                 ...
                                                 @OptionalParam(name = ResearchSubject.SP_DATE) DateRangeParam date,
                                                 @OptionalParam(name = ResearchSubject.SP_IDENTIFIER) TokenAndListParam identifier,
                                                 @OptionalParam(name = ResearchSubject.SP_INDIVIDUAL) ReferenceAndListParam individual,
                                                 @OptionalParam(name = ResearchSubject.SP_PATIENT) ReferenceAndListParam patient,
                                                 @OptionalParam(name = ResearchSubject.SP_STATUS) TokenAndListParam status,
                                                 @OptionalParam(name = ResearchSubject.SP_STUDY) TokenAndListParam study,
        ...
        searchParams.add(ResearchSubject.SP_DATE, date);
        searchParams.add(ResearchSubject.SP_IDENTIFIER, identifier);
        searchParams.add(ResearchSubject.SP_INDIVIDUAL, individual);
        searchParams.add(ResearchSubject.SP_PATIENT, patient);
        searchParams.add(ResearchSubject.SP_STATUS, status);
        searchParams.add(ResearchSubject.SP_STUDY, study);
        ...
        return requestBundleProvider(searchParams, null, ResourceType.ResearchSubject.name(), request, response, requestDetails);
    }

    @Read(version = true)
    public ResearchSubject readResearchSubject(@IdParam IdType id, RequestDetails requestDetails,
                                               HttpServletRequest request, HttpServletResponse response) {
        return requestResource(id, null, ResearchSubject.class, request, response, requestDetails);
    }
}
