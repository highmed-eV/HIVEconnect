...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    KDS_Diagnose

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /KDSDiagnoseBundle with ehr reference    ${text}    ${example_json}


POST /KDSDiagnoseBundle with ehr reference
...
#    &{resp}             POST    ${BASE_URL}/KDSDiagnoseBundleAQL    body=${payload}
#    Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /KDSDiagnoseBundleAQL with ehr reference    ${openehr_template}

POST /KDSDiagnoseBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
    ...
    &{resp_ehr}=    Load JSON From File    ${KDS_DIAGNOSE_EHR_COMPOSITION}/kds_diagnose_canonical.json
