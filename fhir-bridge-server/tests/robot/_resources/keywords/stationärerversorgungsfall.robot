...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    StationÃ¤rer Versorgungsfall

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /StationärerVersorgungsfallBundle with ehr reference    ${text}    ${example_json}


POST /StationärerVersorgungsfallBundle with ehr reference
    [Arguments]         ${fhir_bundle_name}    ${fhir_bundle_file_name}
     ...
     #    &{resp}             POST    ${BASE_URL}/StationärerVersorgungsfallBundleAQL    body=${payload}
     #                        Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /StationärerVersorgungsfallBundleAQL with ehr reference    ${openehr_template}

POST /StationärerVersorgungsfallBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
    ...
    &{resp_ehr}=    Load JSON From File    ${STATIONÄRER_VERSORGUNGSFALL_EHR_COMPOSITION}/stationärer_versorgungsfall_canonical.json
