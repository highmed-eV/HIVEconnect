...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    KDS_Biobank

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /KDSBiobankBundle with ehr reference    ${text}    ${example_json}


POST /KDSBiobankBundle with ehr reference
...
#    &{resp}             POST    ${BASE_URL}/KDSBiobankBundleAQL   body=${payload}
#                        Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /KDSBiobankBundleAQL with ehr reference    ${openehr_template}

POST /KDSBiobankBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
...
#    currently mocking the aql response from the file
    &{resp_ehr}=    Load JSON From File    ${KDS_BIOBANK_EHR_COMPOSITION}/kds_biobank_canonical.json
