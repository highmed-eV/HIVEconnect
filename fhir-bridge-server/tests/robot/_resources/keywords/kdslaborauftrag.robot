...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    KDS_Laborauftrag

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /KDSLaborauftragBundle with ehr reference    ${text}    ${example_json}


POST /KDSLaborauftragBundle with ehr reference
    [Arguments]         ${fhir_bundle_name}    ${fhir_bundle_file_name}
     ...
     #    &{resp}             POST    ${BASE_URL}/KDSLaborberichtBundleAQL    body=${payload}
     #                        Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /KDSLaborauftragBundleAQL with ehr reference    ${openehr_template}

POST /KDSLaborauftragBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
    ...
    &{resp_ehr}=    Load JSON From File    ${KDS_LABORAUFTRAG_EHR_COMPOSITION}/kds_laborauftrag_canonical.json
