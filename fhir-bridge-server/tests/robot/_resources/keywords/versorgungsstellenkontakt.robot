...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    Versorgungsstellenkontakt

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /VersorgungsstellenkontaktBundle with ehr reference    ${text}    ${example_json}


POST /VersorgungsstellenkontaktBundle with ehr reference
    [Arguments]         ${fhir_bundle_name}    ${fhir_bundle_file_name}
     ...
     #    &{resp}             POST    ${BASE_URL}/VersorgungsstellenkontaktBundleAQL    body=${payload}
     #                        Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /VersorgungsstellenkontaktBundleAQL with ehr reference    ${openehr_template}

POST /VersorgungsstellenkontaktBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
    ...
    &{resp_ehr}=    Load JSON From File    ${VERSORGUNGSSTELLENKONTAKT_EHR_COMPOSITION}/versorgungsstellenkontakt_canonical.json
