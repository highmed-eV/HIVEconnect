...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    KDS_Prozedur

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /KDSProzedurBundle with ehr reference    ${text}    ${example_json}


POST /KDSProzedurBundle with ehr reference
    [Arguments]         ${fhir_bundle_name}    ${fhir_bundle_file_name}
     ...
     #    &{resp}             POST    ${BASE_URL}/KDSProzedurBundleAQL    body=${payload}
     #                        Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /KDSProzedurBundleAQL with ehr reference    ${openehr_template}

POST /KDSProzedurBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
    ...
    &{resp_ehr}=    Load JSON From File    ${KDS_PROZEDUR_EHR_COMPOSITION}/kds_prozedur_canonical.json
