...
validate content response_aql - 201
    ...
    Should Be Equal As Strings    ${template_id}[0]    Studienteilnahme

create fhir bundle
    [Arguments]         ${text}    ${example_json}
    POST /StudienteilnahmeBundle with ehr reference    ${text}    ${example_json}


POST /StudienteilnahmeBundle with ehr reference
    [Arguments]         ${fhir_bundle_name}    ${fhir_bundle_file_name}
     ...
     #    &{resp}             POST    ${BASE_URL}/StudienteilnahmeBundleAQL    body=${payload}
     #                        Output Debug Info To Console

create openehr aql
    [Arguments]         ${openehr_template}
    POST /StudienteilnahmeBundleAQL with ehr reference    ${openehr_template}

POST /StudienteilnahmeBundleAQL with ehr reference
    [Arguments]         ${openehr_template}
    ...
    &{resp_ehr}=    Load JSON From File    ${STUDIENTEILNAHME_EHR_COMPOSITION}/studienteilnahme_canonical.json
